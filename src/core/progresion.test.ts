...
import {
  evaluarProgresiónFase,
  calcularCuadranteTensionControl,
} from './progresion'
...
beforeAll(() => {
  ...
  if (!validacion.exito) throw new Error('Programa inválido')
...
describe('progresión', () => {
  describe('evaluación de criterios', () => {
    it('evalúa criterios sin errores', () => {
      ...
      const evaluacion = evaluarProgresiónFase(
        estado,
        programa,
        1,
        '2026-09-20',
    ...
    it('tensión basal cumplida <= 6', () => {
      ...
      const tensionCriterio = evaluacion.criterios.find(
        (c) => c.tipo === 'tension_basal_max',
      )
    ...
    it('registros con tensión baja >= 3', () => {
      ...
      const registrosBajoCriterio = evaluacion.criterios.find(
        (c) => c.tipo === 'registros_con_tension_baja',
      )
  ...
  describe('progresión general', () => {
    it('puede_avanzar si todos los criterios se cumplen', () => {
      ...
      expect(
        evaluacion.criterios.some((c) => c.resultado === 'cumplido'),
      ).toBe(true)
    })
