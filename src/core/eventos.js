...
export function reducirEventos(eventos) {
    const estado = {
        fase_actual: 1,
        dias: new Map(),
        historial_fases: [],
...
function obtenerOInicializarDia(estado, fecha) {
...
export function sonEstadosIguales(a, b) {
    if (a.fase_actual !== b.fase_actual)
        return false;
    if (a.dias.size !== b.dias.size)
        return false;
    for (const [fecha, diaA] of a.dias) {
        const diaB = b.dias.get(fecha);
        if (!diaB)
            return false;
        if (diaA.ejercicios_marcados.size !== diaB.ejercicios_marcados.size)
            return false;
        for (const id of diaA.ejercicios_marcados) {
            if (!diaB.ejercicios_marcados.has(id))
                return false;
        }
