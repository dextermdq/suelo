...
import { validarPrograma, programaSchema, Programa } from '../../config/esquema'

describe('esquema del programa', () => {
  let programa: unknown
  ...
  describe('fases', () => {
    it('fases están en orden creciente', () => {
      const resultado = validarPrograma(programa)
      const datos = resultado.datos as Programa
  ...
  describe('ejercicios', () => {
    ...
    it('ejercicios Kegel no están en Fase 1 ni 2 si son de fuerza', () => {
      ...
      const kegelsFuerza = datos.ejercicios.filter(
        (e) => e.id.includes('kegel') && e.pilar === 'up-training',
      )
